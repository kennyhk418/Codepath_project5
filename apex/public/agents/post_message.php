<?php
  ...
  if(isset($_POST['submit'])) {
    ...
    $receipient_id = $_GET['id'];

    $sender_result = find_agent_by_id($receipient_id);
    $agent = db_fetch_assoc($sender_result);

    $sender = $current_user;

    $encrypted_text = $_POST['plain_text']? pkey_encrypt($_POST['plain_text'], $agent['public_key']) : '';
    $signature = create_signature($_POST['plain_text'], $agent['private_key']);
