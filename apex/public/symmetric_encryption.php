<?php
  ...
  $decode_key = '';
  $decrypted_text = '';
  $encode_algorithm = (isset($_POST['encode_algorithm']))?
                         $_POST['encode_algorithm'] : "AES-256-CBC";
  if(isset($_POST['submit'])) {
    ...
    if(isset($_POST['encode_key'])) {
      ...
      $encrypted_text = key_decrypt($cipher_text, $decode_key, $_POST['decode_algorithm']);
    }
  }
        ...
        <div>
          ...
          <select name="encode_algorithm">
            ...
            <option value="AES-128-CBC"
              <?php if ($encode_algorithm == "AES-128-CBC")
                        echo "selected = true";
              ?>> AES-128-CBC
            </option>
            <option value="AES-192-CBC"
              <?php if ($encode_algorithm == "AES-192-CBC")
                        echo "selected = true";
              ?>> AES-192-CBC
            </option>
            <option value="DES-EDE3-CBC"
              <?php if ($encode_algorithm == "DES-EDE3-CBC")
                        echo "selected = true";
              ?>> DES-EDE3-CBC
            </option>
            <option value="BF-CBC"
              <?php if ($encode_algorithm == "BF-CBC")
                        echo "selected = true";
              ?>> BF-CBC
            </option>
          </select>
