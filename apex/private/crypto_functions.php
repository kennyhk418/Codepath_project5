...
function key_encrypt($string, $key, $cipher_method=CIPHER_METHOD) {
  ...
  global $key_length;
  $key = str_pad($key, $key_length[$cipher_method], '*');

  // Trim the key to be max length 56 for BF-CBC
  if($cipher_method == "BF-CBC" && strlen($key) > 56)
    $key = substr($key, 56);
...
function key_decrypt($string, $key, $cipher_method=CIPHER_METHOD) {
  ...
  // Decrypt
  $decrypted = openssl_decrypt($cipher_text , $cipher_method, $key, OPENSSL_RAW_DATA, $iv);
  return $decrypted;
}
